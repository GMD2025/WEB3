...
class UnoGame implements Game {
  ...
  constructor(
    ...
    if (opts?.roundFromMemento) {
      this._currentRound = createRoundClassFromMemento(
        opts.roundFromMemento,
        this.shuffler,
      ) as RoundClass;
      this._currentRound.onEnd(({ winner }) =>
        this.onRoundEnd(winner, this._currentRound!),
      );
    } else if (opts?.startRound !== false) {
      const dealer = Math.floor(this.randomizer(this.players.length));
      this._currentRound = new RoundClass(
        this.players,
        dealer,
        this.shuffler,
        this.cardsPerPlayer,
